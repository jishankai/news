...
class PostController extends Controller
{
    ...
    {
        $latest = Yii::app()->db->createCommand("SELECT id, title, outline, created_at, updated_at, author, category, price, (SELECT COUNT(*) FROM images i WHERE i.post_id=p.id) AS images_count, thumbnail, free, isDeleted FROM posts p WHERE publish=1 ORDER BY id DESC LIMIT 20")->queryAll();
    ...
    public function actionPosts($id=NULL)
    {
        if (isset($id)) {
            ...
            $posts = Yii::app()->db->createCommand("SELECT  id, title, outline, created_at, updated_at, author, category, price, (SELECT COUNT(*) FROM images i WHERE i.post_id=p.id) AS images_count, thumbnail, free, isDeleted FROM posts p")->queryAll();
    ...
    {
        $previous = Yii::app()->db->createCommand("SELECT  id, title, outline, created_at, updated_at, author, category, price, (SELECT COUNT(*) FROM images i WHERE i.post_id=p.id) AS images_count, thumbnail, free, isDeleted FROM posts p WHERE id<$id and publish!=0 ORDER BY id DESC LIMIT 20")->queryAll();
